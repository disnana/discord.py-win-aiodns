...
[English README](https://github.com/disnana/discord.py-win-aiodns-fix/blob/main/README.md)
...
fallback を使う `auto`、`public`、`custom` は、到達不能な DNS サーバーで起動が
長時間止まらないよう、各段階で 1 秒・1 回だけ aiodns を試します。厳密な
`aiodns` モードだけは、aiodns 本来の timeout / retry の既定値を維持します。
...
インストールします。本番環境では、依存関係の最新セキュリティ修正を利用できる
...
[CONTRIBUTING.md](https://github.com/disnana/discord.py-win-aiodns/blob/main/CONTRIBUTING.md) を参照してください。セキュリティ報告は
[SECURITY.md](https://github.com/disnana/discord.py-win-aiodns/blob/main/SECURITY.md) に従ってください。
